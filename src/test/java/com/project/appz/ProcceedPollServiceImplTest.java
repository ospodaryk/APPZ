...
public class ProcceedPollServiceImplTest {
    ...
    void testSavePollResults_ValidData_NoExceptions() {
        long pollId = 1L;
        ResponsePoll responsePoll = ModelUtils.getResponsePoll();
        Mockito.when(pollRepository.findById(pollId)).thenReturn(Optional.of(ModelUtils.getPoll()));

        service.savePollResults(pollId, responsePoll);
    }

    @Test
    void testSavePollResults_NonExistentPoll_ThrowsNotFoundException() {
        long nonExistentPollId = 123L;
        ResponsePoll responsePoll = new ResponsePoll();
        assertThrows(NullPointerException.class, () -> service.savePollResults(nonExistentPollId, responsePoll));
    }

    @Test
    void testSavePollResults_NoData_ThrowsIllegalArgumentException() {
        long pollId = 1L;
        ResponsePoll responsePoll = null;
        assertThrows(NullPointerException.class, () -> service.savePollResults(pollId, responsePoll));
    }
